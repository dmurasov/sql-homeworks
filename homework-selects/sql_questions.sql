...
SELECT department_name, AVG(salary) AS average_salary, COUNT(employee_id) AS number_of_employees 
FROM departments 
    LEFT JOIN ( 
...
SELECT employee_id, (first_name || ' ' || last_name) AS name_employee
FROM employees
WHERE salary > (
    SELECT AVG(salary)
    FROM employees
...
SELECT (first_name || ' ' || last_name) AS name_employee, employee_id, salary
FROM employees
WHERE manager_id = (
...
SELECT department_id, (first_name || ' ' || last_name) AS full_name, job_title, department_name
FROM employees
    INNER JOIN jobs USING (job_id)
...
SELECT (first_name || ' ' || last_name) AS full_name, job_title, MAX(start_date) AS starting_date, MAX(end_date) as ending_date
FROM job_history jh
    INNER JOIN employees e ON(e.employee_id = jh.employee_id) 
    INNER JOIN jobs jb ON(e.job_id = jb.job_id) 
...
SELECT employee_id, (first_name || ' ' || last_name) AS name_employee, salary 
FROM employees 
WHERE salary > (
    SELECT AVG(salary)
    FROM employees
...
SELECT employee_id, (first_name || ' ' || last_name) AS name_employee, job_title 
FROM employees 
    LEFT JOIN jobs USING(job_id)
WHERE salary < ANY(
        SELECT salary 
